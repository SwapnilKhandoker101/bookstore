...
const createProduct= async (req:Request,res:Response)=>{
    try{
        ...
        const result=await ProductServices.createProductIntoDB(zodparsedData as TProduct)
        res.status(200).json({
            success:true,
            data:result
        })
...
const updateProductFromDB=async (req:Request,res:Response):Promise<void>=>{
    try{
        ...
        // }
        // const objectId:Types.ObjectId=new mongoose.Types.ObjectId(_id);
        const result=await ProductServices.updateProductFromDB(productId,price,quantity)
        console.log(result)
